...
export class UniqueXTree<Key, Elem>
{
    ...
    /* ---------------------------------------------------------
        CONSTRUCTOR
    --------------------------------------------------------- */
    public constructor(keyGetter: (elem: Elem) => Key, keyComp: Comparator<Key>)
    {
        this.root_ = null;
    ...
    public nearest(key: Key): XTreeNode<Elem> | null
    {
        ...
        while (true) // UNTIL MEET THE MATCHED VALUE OR FINAL BRANCH
        {
            ...
            // COMPARE
            if (this.key_comp_(key, this.key_getter_(ret.value)))
                child = ret.left;
            else if (this.key_comp_(this.key_getter_(ret.value), key))
                child = ret.right;
            else
